...
uint256 hashGenesisBlock("0x3b222f933b5192d1fcf1148fc35bb05a2380be5d66312ceb109b3fbfba74e7c0");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // GollumCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "GollumCoin Signed Message:\n";
...
{
    // GollumCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 50 * COIN; //50 GLM per block

	if(nHeight<2881) { //2 days bonus for early adopters
	nSubsidy *= 3;
	}
    ...
    nSubsidy >>= (nHeight / 43200); // GollumCoin: halving every month

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 60 * 60; // GollumCoin: once per hour diff adjust
static const int64 nTargetSpacing = 60; // GollumCoin: 1 minute blocks
static const int64 nInterval = nTargetTimespan / nTargetSpacing; //retarget 60 blocks based
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (fTestNet) {
                if (pindexLast->nHeight+1 >= 2880) { DiffMode = 2; } 
        }
        else {
                if (pindexLast->nHeight+1 >= 2881) { DiffMode = 2; } //switch after 2 days
        }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // GollumCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // GollumCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x3b222f933b5192d1fcf1148fc35bb05a2380be5d66312ceb109b3fbfba74e7c0");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "March 23rd, Sméagol and Déagol went fishing in the Gladden Fields north of Lothlórien";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("") << OP_CHECKSIG; //key for signed bdcst msgs
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1396027222;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2213759;
        ...
        assert(block.hashMerkleRoot == uint256("0xb31b4b28689123588f7b08cb73d4cb2d26a44dd3094af82e2e14ca46e4cb55b5"));
        block.print();
        //assert(hash == hashGenesisBlock);

   // If genesis block hash does not match, then generate new genesis hash.
        if (false && block.GetHash() != hashGenesisBlock) //make false after genesis
        {
...
unsigned char pchMessageStart[4] = { 0xaf, 0xaf, 0xaf, 0xaf }; // GollumCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GollumCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GollumCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GollumCoinMiner(CWallet *pwallet)
{
    printf("GollumCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("GollumCoin-miner");
    ...
    try { loop {
        ...
        printf("Running GollumCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GollumCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GollumCoinMiner, pwallet));
}
